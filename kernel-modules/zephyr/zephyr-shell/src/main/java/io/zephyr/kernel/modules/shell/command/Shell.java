...
import io.zephyr.kernel.modules.shell.console.*;
...
public class Shell implements Invoker, Remote {
  ...
  public Result invoke(Parameters parameters) throws RemoteException {
    ...
    val shell = new CommandLine(delegate).setUnmatchedArgumentsAllowed(true);
    try {
      shell.execute(parameters.formals());
    } catch (Exception ex) {
      log.log(Level.WARNING, "Command failed.  Reason: ", ex);
      shell.usage(System.out);
    }
    return null;
  }
